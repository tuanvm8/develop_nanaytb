@section('headerTitle', 'Quản trị - ' . ($isUpdate ? 'Cập nhập' : 'Tạo mới') . ' bài viết')
...
@section('templateContent')
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">
                            <a href="{{ route('admin.post.index') }}" class="text-decoration-none">Danh sách bài viết</a>
                        </li>
    ...
    <div class="bg-white mx-3 p-3 shadow-sm">
        ...
        <div class="d-flex justify-content-between mb-3">
            <h1 class="fs-3 m-0">{{ $isUpdate ? 'Cập nhập' : 'Tạo mới' }}  bài viết </h1>
        ...
        <div class="">
            <form action="{{ isset($post) ? route('admin.post.update', ['id' => $post->id]) : route('admin.post.create') }}"
                        ...
                        <div class="d-flex">
                            <div class="position-relative border rounded me-3">
                                @if (isset($post) && $post->image)
                                    <img src="{{ $post->image }}" class="add-image" id="imgPreview" style="max-width: 300px">
                        ...
                        <div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="status" id="active" value="1"
                                    {{ old('status') ? (old('status') == 1 ? 'checked' : '') : ($isUpdate ? ($post->status == 1 ? 'checked' : '') : 'checked') }}>
                                <label class="form-check-label" for="active">Hoạt động</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="status" id="inactive" value="2"
                                    {{ old('status') ? (old('status') == 2 ? 'checked' : '') : ($isUpdate ? ($post->status == 2 ? 'checked' : '') : '') }}>
                ...
                <div class="text-end">
                    <button type="submit" id="btn-button" class="btn btn-{{ isset($post) ? 'primary' : 'success' }}">
                        @isset($post)
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor"
            ...
            @isset($post)
                ...
                <div class="d-flex justify-content-end">
                    <form action="{{ route('admin.post.delete', ['id' => $post->id]) }}" id="form-{{ $post->id }}"
                        method="POST">
                        @csrf
                        <button type="button" class="btn btn-danger delete" data-value="{{ $post->id }}">
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor"
