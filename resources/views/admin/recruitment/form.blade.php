@section('headerTitle', 'Quản trị - ' . ($isUpdate ? 'Cập nhập' : 'Tạo mới') . ' bài viết tuyển dụng')
...
@section('templateContent')
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">
                            <a href="{{ route('admin.recruitment.index') }}" class="text-decoration-none">Danh sách bài viết tuyển dụng</a>
                        </li>
    ...
    <div class="bg-white mx-3 p-3 shadow-sm">
        ...
        <div class="d-flex justify-content-between mb-3">
            <h1 class="fs-3 m-0">{{ $isUpdate ? 'Cập nhập' : 'Tạo mới' }}  bài viết tuyển dụng </h1>
            <a href="{{ route('admin.recruitment.index') }}" class="btn btn-primary">
        ...
        <div class="">
            <form action="{{ isset($item) ? route('admin.recruitment.update', ['id' => $item->id]) : route('admin.recruitment.create') }}"
                enctype="multipart/form-data" method="post" id="post-form">
                @csrf
                <div class="row mb-3">
                    <div class="col-12">
                        <label for="title" class="form-label">Tiêu đề<span class="text-danger">*</span></label>
                        ...
                        <div class="d-flex">
                            <div class="position-relative border rounded me-3">
                                @if (isset($item) && $item->image)
                                    <img src="{{ $item->image }}" class="add-image" id="imgPreview" style="max-width: 300px">
                ...
                <div class="row mb-3">
                    <div class="col-sm-6">
                        <label for="type" class="form-label">Thể loại</label>
                        <input type="text" class="form-control" id="type" name="type"
                            value="{{ old('type') ?? (isset($item) ? $item->type : '') }}">
                        <p class="help is-danger text-danger">{{ $errors->first('type') }}</p>
                    </div>
                    <div class="col-sm-6">
                        <label for="location" class="form-label">Địa chỉ</label>
                        <input type="text" class="form-control" id="location" name="location"
                            value="{{ old('location') ?? (isset($item) ? $item->location : '') }}">
                        <p class="help is-danger text-danger">{{ $errors->first('location') }}</p>
                    </div>
                </div>
                <div class="row mb-3">
                    <div class="col-6">
            ...
            @isset($item)
                ...
                <div class="d-flex justify-content-end">
                    <form action="{{ route('admin.recruitment.delete', ['id' => $item->id]) }}" id="form-{{ $item->id }}"
                        method="POST">
                        @csrf
                        <button type="button" class="btn btn-danger delete" data-value="{{ $item->id }}">
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor"
