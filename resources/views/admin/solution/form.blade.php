@section('headerTitle', 'Quản trị - ' . ($isUpdate ? 'Cập nhập' : 'Tạo mới') . ' bài viết giải pháp')
...
@section('templateContent')
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">
                            <a href="{{ route('admin.solution.index') }}" class="text-decoration-none">Danh sách bài viết giải pháp</a>
                        </li>
    ...
    <div class="bg-white mx-3 p-3 shadow-sm">
        ...
        <div class="d-flex justify-content-between mb-3">
            <h1 class="fs-3 m-0">{{ $isUpdate ? 'Cập nhập' : 'Tạo mới' }}  bài viết giải pháp</h1>
            <a href="{{ route('admin.solution.index') }}" class="btn btn-primary">
        ...
        <div class="">
            <form action="{{ isset($solution) ? route('admin.solution.update', ['id' => $solution->id]) : route('admin.solution.create') }}"
                enctype="multipart/form-data" method="post" id="post-form">
                @csrf
                <div class="row mb-3">
                    <div class="col-12">
                        <label for="title" class="form-label">Tên bài viết<span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="name" name="name"
                            value="{{ old('name') ?? (isset($solution) ? $solution->name : '') }}">
                ...
                <div class="row mb-3">
                    <div class="col-12">
                        ...
                        <input type="text" class="form-control" id="slug" name="slug"
                            value="{{ old('slug') ?? (isset($solution) ? $solution->slug : '') }}">
                        ...
                        <div class="d-flex">
                            <div class="position-relative border rounded me-3">
                                @if (isset($solution) && $solution->image)
                                    <img src="{{ $solution->image }}" class="add-image" id="imgPreview" style="max-width: 300px">
                ...
                <div class="row mb-3">
                    <div class="col-12">
                        ...
                        <textarea id="editor-area" class="form-control" name="content">
                            {{ old('content') ?? (isset($solution) ? $solution->content : '') }}
                        </textarea>
                        ...
                        <div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="status" id="active" value="1"
                                    {{ old('status') ? (old('status') == 1 ? 'checked' : '') : ($isUpdate ? ($solution->status == 1 ? 'checked' : '') : 'checked') }}>
                                <label class="form-check-label" for="active">Hoạt động</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="status" id="inactive" value="2"
                                    {{ old('status') ? (old('status') == 2 ? 'checked' : '') : ($isUpdate ? ($solution->status == 2 ? 'checked' : '') : '') }}>
                ...
                <div class="text-end">
                    <button type="submit" id="btn-button" class="btn btn-{{ isset($solution) ? 'primary' : 'success' }}">
                        @isset($solution)
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor"
                        ...
                        @endisset
                        {{ isset($solution) ? 'Cập nhập' : 'Tạo mới' }}
                    </button>
                </div>
            </form>
            @isset($solution)
                ...
                <div class="d-flex justify-content-end">
                    <form action="{{ route('admin.solution.delete', ['id' => $solution->id]) }}" id="form-{{ $solution->id }}"
                        method="POST">
                        @csrf
                        <button type="button" class="btn btn-danger delete" data-value="{{ $solution->id }}">
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor"
