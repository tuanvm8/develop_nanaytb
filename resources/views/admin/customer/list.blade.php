@section('headerTitle', 'Quản trị - Danh sách nguời dùng')
                        ...
                        <tr>
                            ...
                            <td class="text-center" style="padding: 7px 0 0 42px;">
                                <div class="form-check form-switch">
                                    <form action="{{ route('admin.customer.status', ['id' => $item->id]) }}" method="POST"
                                        id="form-status-{{ $item->id }}">
                                        @csrf
                                        <input type="checkbox"
                                            class="form-check-input status {{ $item->status == 2 ? 'active-item' : 'deactive-item' }}"
                                            name="status" data-value="{{ $item->id }}"
                                            id="status-{{ $item->id }}" {{ $item->status == 2 ? 'checked' : '' }}>
                            ...
                            <td class="" style="padding: 7px 0 0 42px;">
                                <div class="form-check form-switch">
                                    <form action="{{ route('admin.customer.supported', ['id' => $item->id]) }}"
                                        method="POST" id="form-isVerified-{{ $item->id }}">
                                        @csrf
                                        <input type="checkbox"
                                            class="form-check-input isVerified {{ $item->is_verified == 2 ? 'active-item' : 'deactive-item' }}"
                                            name="is_verified" data-value="{{ $item->id }}"
                                            id="isVerified-{{ $item->id }}"
                                            {{ $item->is_verified == 2 ? 'checked' : '' }}>
                                        <label class="form-check-label" type="hidden"
                                            for="isVerified-{{ $item->id }}"></label>
                                    </form>
                                </div>
                            </td>
