            ...
            <div class="col-lg-6 col-md-7 col-sm-2">
                <div class="box-login" style="box-shadow: 1px 1px 10px 1px #cccc;border-radius: 30px;">
                    <div class="row d-flex justify-content-center align-items-center pb-4 pt-2" >
                        <div class=" col-lg-4 col-md-4 col-4  d-flex justify-content-center align-items-center pl-2 " style="padding-left: 20px;">
                            <img class="rounded-circle img-fluid p-2 " src="{{ asset('images/image-login.png') }}" alt="" style="border: 1px solid #0dcaf0; "/>
                        </div>
                        <div class=" col-lg-8 col-md-8 col-8">
                            <p class="form-label">{{ Auth::user()->username }}</p>
                            <p class="form-label" style="font-weight: 400;">{{ Auth::user()->email }}</p>
                            <p class="form-label" style="color: red;">Số dư: <span class="user_point formattedNumber" style="color: red;">{{ Auth::user()->point }}</span></p>
                        </div>
                    </div>
                    ...
                    <form method="post" action="{{ route('withdraw_money.post') }}" id="form-ruttien">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" required="" name="point"
                                placeholder="Nhập số tiền muốn rút" value="{{ old('point') ?? Auth::user()->point }}" style="pointer-events: none;">
                        ...
                        <div class="mb-3">
                            ...
                            <p class="help is-danger text-danger confirm_password">{{ $errors->first('account_number') }}</p>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Ngân hàng</label>
                            <input type="text" class="form-control" required="" name="name_bank"
                                placeholder="Nhập tên ngân hàng" value="{{ old('name_bank') }}">
                            <p class="help is-danger text-danger confirm_password">{{ $errors->first('name_bank') }}</p>
                        </div>
                        <div class="mb-3">
                        ...
                        @else
                        <p class="help is-danger text-danger confirm_password">Tài khoản chưa đủ mức để được rút tiền</p>
                        @endif
                    </form>
                </div>
            </div>
